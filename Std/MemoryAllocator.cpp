...
MemoryAllocator::MemoryAllocator(Bytes heap) : m_heap(heap) {
...
u8 *MemoryAllocator::allocate(usize size, bool debug_override, void *address) {
  ...
  for (Node *entry = m_freelist; entry; entry = entry->m_next) {
    if (entry->m_size >= size + sizeof(Node)) {
      auto *next =
          reinterpret_cast<Node *>(entry->m_data + round_to_word(size));
  ...
  // Allocation failed: return nullptr so caller can handle it (e.g. expand
  // heap)
  return nullptr;
}

void MemoryAllocator::add_memory(Bytes bytes) {
  if (bytes.size() <= sizeof(Node))
    return;

  // Construct a free node at the start of the new memory
  Node *node = reinterpret_cast<Node *>(bytes.data());
  node->m_size = bytes.size() - sizeof(Node);

  // Use deallocate to insert/merge it into the freelist
  deallocate(node->m_data, false, __builtin_return_address(0));
}

void MemoryAllocator::deallocate(u8 *pointer, bool debug_override,
                                 void *address) {
  if (pointer == nullptr)
    return;
  ...
  for (auto *entry = m_freelist; entry; entry = entry->m_next) {
    if (entry > target_entry) {
      ...
      if (previous &&
          previous->m_data + previous->m_size == (u8 *)target_entry) {
...
u8 *MemoryAllocator::reallocate(u8 *pointer, usize size, bool debug_override,
                                void *address) {
  if (address == nullptr)
    address = __builtin_return_address(0);
