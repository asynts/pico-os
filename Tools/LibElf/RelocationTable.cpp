...
namespace Elf
{
    ...
    void RelocationTable::initialize(Generator& generator)
    {
        m_rel_index = generator.create_section(fmt::format(".rel{}", m_name_suffix), SHT_REL, 0);
        auto& rel_section = generator.section(*m_rel_index);
        rel_section.sh_entsize = sizeof(Elf32_Rel);
        rel_section.sh_link = m_symtab_index;
        rel_section.sh_info = m_target_index;

        m_rela_index = generator.create_section(fmt::format(".rela{}", m_name_suffix), SHT_RELA, 0);
    ...
    void RelocationTable::finalize(Generator& generator)
    {
        assert(!m_finalized);
        m_finalized = true;

        generator.write_section(m_rel_index.value(), m_rel_stream);
        generator.write_section(m_rela_index.value(), m_rela_stream);
    }
}
