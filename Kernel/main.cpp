...
void create_shell_process() {
  auto &shell_file = dynamic_cast<Kernel::FlashFile &>(
      Kernel::FileSystem::lookup("/bin/Shell.elf"));
...
static HardwareSpinLock
    s_sw_spinlock_hw_lock((volatile u32 *)0xd0000100);                // Lock 0
...
static HardwareSpinLock
    s_scheduler_hw_lock((volatile u32 *)0xd0000110); // Lock 4
...
void boot() {
  ...
  Kernel::Scheduler::initialize(move(thread), s_scheduler_hw_lock);
...
void boot_with_scheduler() {
  ...
  dynamic_cast<Kernel::VirtualDirectory &>(root_file).m_entries.set(
      "example.txt", &example_file);
