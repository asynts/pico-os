...
namespace Kernel::Drivers
{
    ...
    //
    // -   The UART chip can write data and decrement it's transfer count.
    //
    // -   The UART can fire an interrupt when the transfer is complete.
    //
    ...
    //     This does not hurt, because the interrupt is triggered when the transfer has been completed.
    //
    // -   The kernel always checks the interrupt counter to verify that no interrupt occured and retries if necessary.
    //
    // When the kernel starts up, the transfer count will be zero.
    // Then we write some data into the buffer and update the transfer count.
    // If we want to write more data before this transfer is completed, we can not update the transfer count atomically.
    // Instead, we simply increment the producer offset, the interrupt handler will then update the transfer count.
    //
    // Assumptions:
    //
    ...
    // Known Bugs:
    //
    // -   If 4 GiB are transfered, then m_producer_offset will roll around and we run into issues.
    //
